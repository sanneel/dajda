...
export async function startTicketPurchase(
  ...
  if (prediction.author?.userId === actor.userId) {
    throw new AppError(ERROR_CODES.VALIDATION_ERROR, 'საკუთარ ბილეთს ვერ შეიძენთ.');
  }

  const existing = await prisma.predictionPurchase.findUnique({
    where: {
      userId_predictionId: { userId: actor.userId, predictionId: prediction.id },
    },
  ...
  const session = await provider.createCheckoutSession({
    ...
    returnUrl: `${env.APP_URL}/free/${prediction.id}?order=${orderId}`,
